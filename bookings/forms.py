...
class BookingsForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if num_guests and accomodation_name:
            ...
            elif num_guests == 2 and str(
                accomodation_name) not in [
                    'Honeymoon Pod', 'Pod Cupla']:
                self.add_error(
                    None,
                    "Honeymoon Pod or Pod Cupla are fantastic options" /
                    " - we strongly recommend those instead for 2 guests")
            elif num_guests == 1 and str(
                accomodation_name) not in [
                    'Honeymoon Pod', 'Pod Cupla']:
                self.add_error(
                    None,
                    "Honeymoon Pod or Pod Cupla are fantastic options" /
                    "- we strongly recommend those instead for 1 guest")
            elif num_guests == 3 and str(
                accomodation_name) not in [
                    'Family Cabin', 'Teach Papa Rua', 'Pod Almighty']:
                self.add_error(
                    None,
                    "This pod only accomodates maximum two guests")
            elif num_guests == 4 and str(
                accomodation_name) not in [
                    'Teach Papa Rua']:
                self.add_error(
                    None,
                    "Teach Papa Rua is a fantastic option for 4 people" /
                    " - we strongly recommend this pod instead")
            elif num_guests == 5 and str(
                accomodation_name) not in [
                    'Pod Almighty']:
                self.add_error(
                    None,
                    "Pod Almighty is a fantastic option for 5 people" /
                    "- we strongly recommend this pod instead")
